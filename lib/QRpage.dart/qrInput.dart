  ...
  Future<List<SiswaData>> fetchdatasiswa(String query) async {
    ...
    try {
      final response = await http.get(
        Uri.parse("http://34.101.47.131/siswaapi?search=${widget.result}"),
        headers: {
          'Cookie': token,
        },
      );
...
// Fungsi untuk mengatur nilai awal dropdown
...
// Fungsi untuk menyimpan nilai ke SharedPreferences
  void saveToSharedPreferences(Map<String, dynamic>? newValue) {
    if (newValue != null) {
      prefs.setString('jenis', json.encode(newValue));
    }
  }

  // Map<String, dynamic>? getDefaultDropdownitem() {
  //   if (selectedDropdownitem2 != null) {
  //     for (Map<String, dynamic> item in dropdownItems2) {
  //       if (item['jenis'] == selectedDropdownitem2?['jenis']) {
  //         return item;
  //       }
  //     }
  //   }
